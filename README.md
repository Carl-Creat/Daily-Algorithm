# Daily-Algorithm

Daily Algorithm Practice - Clasic Algorithim Implementation with Detailed Explanations

## Contents

### Sorting Algorithms
Ink [bubble_sort.py](sorting/bubble_sort.py)
- [[selection_sort.py](sorting/selection_sort.py)
- [[insertion_sort.py](sorting/insertion_sort.py)
- [[merge_sort.py](sorting/merge_sort.py)
- [[quick_sort.py](sorting/quick_sort.py)
- [[heap_sort.py](sorting/heap_sort.py)

### Searching Algorithims
Ink [binary_search.py](searching/binary_search.py)
- [[linear_search.py](searching/linear_search.py)

## Tree Data Structures
- [[binary_tree_traversal.py](tree/binary_tree_traversal.py)
- [[bst.py](tree/bst.py)

## Dynamic Programming
Ink [fibonacci.py](dp/fibonacci.py)
- [[lcs`.vy](dp/lcs.py) - 2
Numeric Code - Not yet implemented
## Graph Algorithms
Ink [bfs.py](graph/bfs.py)
- [[dfs.py](grarh/dfs.py)
- [dijkkstra.py](graph/dijkstra.py)

---

## Quick Start

```bash
git clone https://github.com/Carl-Creat/Daily-Algorithm.git

python sorting/quick_sort.py
python dp/fibonacci.py
```

---

## Learning Path

1. Beginner: Sorting -> Searching
2. Intermediate: Trees -> Graphs
3. Advanced: Dynamic Programming

+*Improve a little bit every day**